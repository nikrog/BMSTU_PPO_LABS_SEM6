...
Request PgRequestRepository::getRequestByID(int id)
{
    ...
    try
    {
        if (this->connection->is_open())
        {
            ...
            if (result.size() > 0)
            {
                resultReq = Request(id, result[0][1].as<int>(), result[0][2].as<int>(), 
                                  result[0][3].as<float>(), result[0][4].as<int>(), result[0][5].as<date_t>(),
                                   State_t(result[0][6].as<int>()), result[0][7].as<int>());
            }
            else
                resultReq = Request();
            curConnect.commit();
        ...
        if (this->connection->is_open())
        {
            ...
            for (size_t i = 0; i < result.size(); i++)
            {
                Request currentReq = Request(result[i][0].as<int>(), result[i][1].as<int>(), result[i][2].as<int>(), 
                                  result[i][3].as<float>(), result[i][4].as<int>(), result[i][5].as<date_t>(),
                                   State_t(result[i][6].as<int>()), result[i][7].as<int>());
